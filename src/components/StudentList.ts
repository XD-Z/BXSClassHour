...
export function StudentList(students: Student[]) {
  ...
  return `
    <div class="p-6">
      <div class="flex justify-between items-center mb-6">
        <h2 class="text-xl font-bold text-gray-800">学员列表</h2>
        <button id="add-student-btn" class="bg-blue-600 text-white px-4 py-2 rounded-lg hover:bg-blue-700 transition-colors">
          + 添加学员
        </button>
      </div>
      ...
      <div class="bg-white rounded-xl shadow-md overflow-hidden">
        <table class="w-full">
          ...
          <tbody id="student-table-body">
            ${students.length === 0 ? `
              <tr>
                <td colspan="11" class="px-4 py-8 text-center text-gray-500">暂无学员数据</td>
              </tr>
            ` : students.map((student, index) => {
              ...
              return `
                <tr class="border-t hover:bg-gray-50 transition-colors ${expired ? 'bg-red-50' : ''}" draggable="true" data-index="${index}" data-id="${student.id}">
                  ...
                  <td class="px-2 py-2 text-center">${student.usedHours}</td>
                  ...
                  <td class="px-2 py-2">
                    <div class="flex gap-1">
                      <button class="edit-student-btn text-blue-600 hover:text-blue-800 text-sm" data-id="${student.id}">编辑</button>
                      <button class="record-hours-btn text-green-600 hover:text-green-800 text-sm" data-id="${student.id}">记课时</button>
                      <button class="delete-student-btn text-red-600 hover:text-red-800 text-sm" data-id="${student.id}">删除</button>
                    </div>
                  </td>
                </tr>
              `;
